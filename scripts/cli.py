...
def _deploy_phase2(
    ...
    try:
        ...
        if accounts:
            ...
            # gogcli needs an encrypted FILE keyring in the headless container.
            # gog_init.py (image) hard-requires GOG_KEYRING_PASSWORD and skips all
            # credential init without it. The password is generated by
            # `setup-google` and stored at the top level of this secret — inject it
            # so the runtime can read/write the refresh token.
            ...
            if keyring_password:
                ...
                log.warning(
                    "Google OAuth secret has no keyring_password — gog credentials "
                    "will NOT initialize in the runtime. Re-run `just setup-google`."
                )
            ...
            log.info(
                "Google OAuth: injecting %d account(s): %s (keyring_password=%s)",
                len(accounts),
                ", ".join(accounts.keys()),
                "set" if keyring_password else "MISSING",
            )
        else:
